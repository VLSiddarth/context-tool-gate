"""demo_ops.py – extended demo with closed‑loop recovery simulation."""
...
from gates.ops_queue import route_action, simulate_recovery, escalate
...
def run_scenario(name, messages, tool_name, tool_extra=None, threshold=None,
    ...
    """
    Run a single scenario with optional closed‑loop recovery simulation.
    """
    ...
    if simulate_recovery_attempt and final_decision != "AUTO":
        ...
        console.print(f"📬 Routing: [bold]{routing['routing_target']}[/bold] (owner: {routing['owner']})")

        recovery_result = simulate_recovery(expected_action["type"], evidence_id)
        console.print(f"⚙️ Recovery attempt: {recovery_result['status']}")

        if recovery_result["status"] == "recovery_failed":
            # Escalate after simulated timeout
            esc = escalate(expected_action["type"], expected_action["owner"], evidence_id)
            console.print(
                f"⏰ Timeout – Escalation: [bold red]{esc['escalation_target']}[/] "
                f"for owner {esc['owner']}"
            )
            # Second trace event
            second_trace = Table(title="Second Trace Event — Recovery Failed", box=box.MINIMAL)
            second_trace.add_column("Field", style="bold cyan")
            second_trace.add_column("Value", style="white")
            second_trace.add_row("Status", esc["status"])
            second_trace.add_row("Action Type", esc["action_type"])
            second_trace.add_row("Evidence ID", esc["evidence_id"])
            second_trace.add_row("Escalation Target", esc["escalation_target"])
            console.print(second_trace)
        else:
            console.print("[green]Recovery successful – no escalation needed.[/green]")
...
def main():
    if hasattr(console, 'print'):
        console.print(Panel(
            ...
            "Freshness → Evidence → Policy + Closed‑Loop Recovery",
            border_style="blue"
        ))

    # Choose one scenario that typically blocks – to show recovery flow.
    # We'll use the stale evidence scenario, but with the mock it may be fresh; we can force threshold 0.01 to guarantee drop.
    run_scenario(
        "Stale Evidence – Closed‑Loop Recovery Demo",
        messages=[
        ...
        threshold=0.01,   # force drop so we always hit recovery
